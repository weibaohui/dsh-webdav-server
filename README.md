...
首次启动令牌自动生成并持久化（`~/.dsh/dsh-webdav-server/settings` 由宿主 settings 服务持久化；settings 服务缺席时退回 `~/.dsh/dsh-webdav-server/token` 文件），可随时在设置页重新生成。
...
> ⚠️ 安全提示：根目录指到哪，挂载者（持有令牌的人）就能读写到哪。别把整个 HOME 指出去；只读共享公开目录时记得开只读模式。
